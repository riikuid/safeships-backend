...
class SafetyPatrolController extends Controller
{
        ...
        try {
            ...
            foreach ($superAdmins as $admin) {
                ...
                Notification::create([
                    ...
                    'reference_type' => 'safety_patrol_approve',
            ...
            if ($user->role === 'super_admin') {
                ...
                if ($superAdminApprovals->every(fn($a) => $a->status === 'approved')) {
                    ...
                    Notification::create([
                        ...
                        'reference_type' => 'safety_patrol_view',
                        'reference_id' => $patrol->id,
                    ]);
                ...
                Notification::create([
                    ...
                    'reference_type' => 'safety_patrol_action',
                    'reference_id' => $patrol->id,
                ]);
    ...
    /**
     * @OA\Get(
     *     path="/api/safety-patrols/report-data",
     *     summary="Get safety patrol report data for charts",
     *     tags={"Safety Patrol"},
     *     security={{"sanctum":{}}},
     *     @OA\Parameter(
     *         name="year",
     ...
     *                     property="labels",
     *                     type="array",
     *                     @OA\Items(type="string", example="2021")
     *                 ),
     *                 @OA\Property(
     *                     property="datasets",
     *                     type="array",
     *                     @OA\Items(
     *                         type="object",
     *                         @OA\Property(property="label", type="string", example="Unsafe Condition"),
     *                         @OA\Property(property="data", type="array", @OA\Items(type="integer", example=10)),
     *                         @OA\Property(property="backgroundColor", type="string", example="#ff6384")
     *                     )
     *                 )
     *             )
     *         )
    ...
    public function reportData(Request $request)
    {
        try {
            ...
            $query = SafetyPatrol::query()->withTrashed();

            if ($year) {
                // Monthly data for the specified year
                $data = $query->select(
                    DB::raw('MONTH(created_at) as month'),
                    DB::raw('SUM(CASE WHEN type = "unsafe_condition" THEN 1 ELSE 0 END) as unsafe_condition'),
                    DB::raw('SUM(CASE WHEN type = "unsafe_action" THEN 1 ELSE 0 END) as unsafe_action')
                )
                ...
                foreach ($data as $row) {
                    ...
                    $unsafeActionData[$monthIndex] = (int) ($row->unsafe_action ?? 0);
                }
                ...
                foreach ($data as $row) {
                    ...
                    if ($yearIndex >= 0 && $yearIndex < 5) {
                        $unsafeConditionData[$yearIndex] = (int) ($row->unsafe_condition ?? 0);
                        $unsafeActionData[$yearIndex] = (int) ($row->unsafe_action ?? 0);
                    }
                }
